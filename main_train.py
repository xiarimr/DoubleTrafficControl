...
def train(
    ...
    try:
        for ep in range(total_epochs):
            ...
            # Use full_restart=True for first epoch, False for subsequent epochs
            ...
            while steps < batch_steps:
                ...
                # Store current global observation (before transition) for critic
            ...
            # bootstrap last value (using final state after last step)
            last_gobs = np.concatenate([o["A"], o["B"]], axis=0).astype(np.float32).reshape(1, -1)
            ...
            if ep % save_every == 0:
                ...
                print(f"Saved checkpoint at ep {ep}: {save_path}")

        # final save
        final_path = manager.save()
        print(f"Training finished, final checkpoint saved: {final_path}")
    finally:
        # Ensure environment is properly closed even if training is interrupted
        env.close()
